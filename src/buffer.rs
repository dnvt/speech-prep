//! Audio buffer types for ML processing
//!
//! Provides `AudioBuffer` for batch audio processing in ML pipelines,
//! complementing streaming audio types.
//!
//! ## Architecture
//!
//! - **AudioChunk**: Streaming-focused, real-time transport
//! - **AudioBuffer** (this module): ML-focused, batch processing
//!
//! ## Design Principles
//!
//! - Zero-copy conversions from AudioChunk where possible
//! - Temporal type integration (AudioDuration)
//! - ML framework compatibility (Candle, PyTorch)
//! - Validation and normalization for ML inference
...
/// Audio buffer for ML processing
///
/// Optimized for batch audio processing in ML pipelines. Provides
/// utilities for normalization, resampling, and format conversion.
///
/// # Example
///
/// ```rust
...
impl AudioBuffer {
    ...
    pub fn from_samples(samples: Vec<f32>, sample_rate: u32) -> Result<Self> {
        ...
        Ok(Self {
            samples,
            sample_rate,
            metadata: Some(AudioMetadata::default()),
        })
    }

    /// Create `AudioBuffer` from `AudioChunk` (zero-copy data)
    ///
    /// Converts streaming `AudioChunk` to batch `AudioBuffer` for ML
    /// processing.
    ///
    /// # Example
    ///

...
impl AudioFixtures {
    /// Create new `AudioFixtures` with default paths relative to the workspace
    /// root.
    ///
    /// Default paths:
    /// - Curated: `<workspace>/datasets/curated/`
    /// - Full dataset: `<workspace>/fixtures/audio/`
    pub fn new() -> Self {
        let workspace_root = workspace_root();
        Self {
            curated_path: workspace_root.join("datasets/curated"),
            full_dataset_path: workspace_root.join("fixtures/audio"),
        }
    }
    ...
    fn try_load_full_dataset(&self, id: &str) -> Result<Option<AudioSample>> {
        if let Some(entry) = self.lookup_manifest_entry(id)? {
            if let Some(original_rel) = entry.get("original_path").and_then(|p| p.as_str()) {
                let original_path = workspace_root().join(original_rel);
...
mod tests {
    ...
    fn test_has_real_datasets_detection() {
        ...
        // Should not panic, just return boolean
        println!("Real datasets available: {}", has_datasets);
        assert!(
            matches!(has_datasets, true | false),
    ...
    fn test_load_curated_sample_if_available() {
        ...
        match sample.metadata.source {
            AudioSource::Curated => {
                println!("ok: loaded from curated dataset");
                assert_eq!(sample.sample_rate, 16000);
                assert!(!sample.audio_data.is_empty());
            }
            AudioSource::FullDataset => {
                println!("ok: loaded from full dataset fallback");
                assert_eq!(sample.sample_rate, 16000);
                assert!(!sample.audio_data.is_empty());
            }
            AudioSource::Synthetic => {
                println!("ok: fallback to synthetic (curated not available)");
                assert_eq!(sample.sample_rate, 16000);
            }
        }
    }
    ...
    fn test_smoke_test_curated_manifest_structure() {
        ...
        if !fixtures.curated_dataset_available() {
            eprintln!("warning: skipping: curated dataset not available");
            return;
        }
        ...
        eprintln!(
            "ok: curated manifest structure validated ({} samples)",
            samples.len()
        );
    }
    ...
    fn test_smoke_test_sample_loading_integrity() {
        ...
        eprintln!(
            "ok: sample loading integrity validated ({} samples checked)",
            10
        );
    }
    ...
    fn test_smoke_test_audio_quality_distribution() {
        ...
        eprintln!(
            "ok: quality distribution validated: {clean_percent:.1}% clean, \
             {moderate_percent:.1}% moderate, {challenging_percent:.1}% challenging"
        );
    }
}

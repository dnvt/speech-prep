...
pub struct VadConfig {
    ...
    ///
    /// **Default**: `AudioTimestamp::EPOCH` (Unix epoch: 1970-01-01 00:00:00 UTC)
    ///
    /// **Effect**: Used for timestamping detected speech segments. Set this
    /// to the actual recording start time for accurate absolute timestamps,
    /// or leave as EPOCH for relative timestamps from start of processing.
    ///
    /// **Use Cases**:
    /// - Real-time streaming: Set to `AudioTimestamp::now()` when starting
    /// - Batch processing: Set to file creation/modification time

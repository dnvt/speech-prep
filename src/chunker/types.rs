...
///
/// Indicates the speech context at chunk start/end, enabling downstream
/// consumers to apply appropriate processing (e.g., cross-fade at transitions).
...
///
/// Represents a segment of audio aligned to speech boundaries, suitable for
/// parallel processing by multiple downstream consumers.
#[derive(Debug, Clone)]
pub struct ProcessedChunk {
    ...
    ///
    /// Derived from VAD analysis. Used for adaptive routing decisions
    /// (high speech ratio → GPU tier, low → CPU tier).
    pub speech_ratio: f32,

    /// RMS energy of the chunk (computed during generation).
    ///
    /// Useful for quality assessment and adaptive processing decisions.
    pub energy: f32,
    ...
    /// Clipped audio may cause distortion in downstream processing.
    ///
    ...
    /// chunk, providing acoustic context for downstream speech processing.
...
impl ProcessedChunk {
    ...
    /// downstream consumers that want to avoid processing overlap regions
    /// twice.

...
use super::artifacts::WavArtifactWriter;
...
impl NoiseReducer {
    ...
    pub fn reduce(&mut self, samples: &[f32], vad_context: Option<VadContext>) -> Result<Vec<f32>> {
        self.reduce_with_artifacts(samples, vad_context, None)
    }

    /// Apply noise reduction and optionally capture QA artifacts.
    pub fn reduce_with_artifacts(
        &mut self,
        samples: &[f32],
        vad_context: Option<VadContext>,
        mut artifacts: Option<&mut NoiseReductionArtifacts<'_>>,
    ) -> Result<Vec<f32>> {
        ...
        if let Some(ref mut recorder) = artifacts {
            recorder.record_input(samples);
        }
...
pub struct NoiseReductionArtifacts<'a> {
    before: Option<&'a mut WavArtifactWriter>,
    after: Option<&'a mut WavArtifactWriter>,
}

impl<'a> NoiseReductionArtifacts<'a> {
    /// Create a recorder for the provided before/after writers.
    pub fn new(
        before: Option<&'a mut WavArtifactWriter>,
        after: Option<&'a mut WavArtifactWriter>,
    ) -> Self {
    ...
    /// Persist the noisy input samples for QA review.
    pub fn record_input(&mut self, samples: &[f32]) {
        if let Some(writer) = self.before.as_deref_mut() {
            if let Err(err) = writer.write_samples(samples) {
                warn!(target: "audio.preprocess.noise_reduction", error = %err, "failed to write input artifact");
            }
        }
    }

    /// Persist the denoised output samples for QA review.
    pub fn record_output(&mut self, samples: &[f32]) {
        if let Some(writer) = self.after.as_deref_mut() {
            if let Err(err) = writer.write_samples(samples) {
                warn!(target: "audio.preprocess.noise_reduction", error = %err, "failed to write output artifact");
            }
        }
    }
}

impl std::fmt::Debug for NoiseReductionArtifacts<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("NoiseReductionArtifacts").finish()
    }
}
...
mod tests {
    ...
    fn test_snr_improvement_white_noise() -> TestResult<()> {
        ...
        eprintln!(
            "SNR improvement: Before={:.1} dB, After={:.1} dB, Improvement={:.1} dB",
